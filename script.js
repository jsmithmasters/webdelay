...
(function(){
  ...
  if(!btn || !app) return;
  // restore last state this session
  try {
    if (sessionStorage.getItem('controlsHidden') === '1') {
  ...
  btn.addEventListener('click', () => {
    ...
    try { sessionStorage.setItem('controlsHidden', hidden ? '1' : '0'); } catch {}
    // Trigger a resize so the canvas can refit if your code listens to it
