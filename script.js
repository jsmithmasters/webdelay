// Delay Coach — iOS Clean build (Responsive Sidebar)
(() => {
  ...
  // Pick canvas dims to match stage area and maintain 16:9 inside it
  function pickDims(){
    const stage = document.querySelector('.stage');
    const rect = stage.getBoundingClientRect();
    let w = Math.max(1, rect.width|0);
    let h = Math.max(1, rect.height|0);
    // Fit 16:9 inside stage
  ...
  resizeObs.observe(document.querySelector('.stage'));
